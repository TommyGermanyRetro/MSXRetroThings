# RTCLCDP - resident RTC/LCD clock display

## RTCLCDP.TSR

A real MemMan 2.4 TSR (background program, not just a `.COM` that exits) for Card A. On install
it behaves like `RTCLCDI.COM` once: discovers `RCX_INTERFACE`/`XIO_IO_EXPANDER`, reads the
PCF8583 RTC, writes date and time to the SEED I2C LCD, registers a PIC interrupt channel - but
then it copies its interrupt-driven refresh logic into a fixed, permanently resident RAM block
and returns to MSX-DOS instead of exiting. From then on, MemMan's own `H.TIMI` hook keeps calling
that resident code on every timer tick, so the LCD clock keeps updating in the background while
any other program runs - no foreground loop, no `<SPACE>` to exit needed.
